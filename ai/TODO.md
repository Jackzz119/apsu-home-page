...
- [x] [BUG] #20 BMI options CTA 文字贴左边：恢复标准按钮 leading inset，11 宽检查文字实际留白。
- [x] [BUG] #21 Menu 打开横跳、关闭按钮错位：只在锁滚动时补偿实际滚动条宽度；按导航栏位置对齐，处理 resize 后滚动锚定，原地开关与恢复验证通过。
- [x] [BUG] #22 语言胶囊底边裁切：固定 48px 高度、flex 对齐并预留上下绘制空间；滚动 / 键盘 / 减动效均验证。

- [x] [BUG] #18 BMI 报错后单位切换需多点：radio 独占完整命中区，按压文字不拦截；375/1440 首次点击、键盘及触控模拟验证通过。
- [x] [BUG] #19 BMI 大分数突然变小：按最终值实测宽度适配，字体 / 字重一致，字号上限 52px；容器变窄 / 恢复、极端大数无横溢出及布局回归通过。
