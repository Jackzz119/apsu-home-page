...
> 本文记录设计真值与语义映射；运行时 `styles/tokens.css` 仍是占位，落地归 P3。偏差候选未经用户确认不替换原稿；状态方案尚未实装。
...
- P1 交付的是读稿与实现依据；UI 尚未实现，不声明像素验收、键盘走查或动效验收通过。P2 可据区块清单设计契约，P3 可据已核 token 开工；缺陷项在对应实施步骤前单独批准。
