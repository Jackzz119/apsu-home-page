...
### 5.0a 展示交互规则（2026-09-17 用户设立）

本项目以展示组件、设计和交互为目标。具有交互外观或交互用途的组件，即使没有真实业务链接、外部 callback 或后端，也应实现 hover / focus / pressed，以及适合该组件的本地选中、切换、展开等可见反馈；不得仅因缺少 link / callback 就取消交互。原本就是非交互的文字、图片、装饰和信息组件不强行按钮化。展示交互不必模拟真实业务完成，也不意味着自动增加页面翻译或问诊后台。

语言标签按本轮用户决定采用可点击的展示切换：保留源稿初始高亮，点击切换本地选中状态，不切换网站语言。跑马灯保留悬停暂停；鼠标点击 Resume 后离开区域立即继续，不能被残留的鼠标焦点持续拦住；键盘焦点仍应保护操作中的内容。
