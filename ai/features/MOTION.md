...
- 2026-09-18 P7：`tests/acceptance/states.spec.ts` 以 DevTools 协议强制伪类，逐个量全页可见控件（1440：65；375 含菜单：69）。口径：链接 / 按钮 / summary 必须 hover + pressed + focus-visible 且指针反馈有过渡；输入 / 单选 / 可聚焦区域只要 focus-visible（PROJECT §11 #10）。扫描前补齐 D-03 的 logo 链接缺口并给 FAQ chevron 底色加过渡；步进按钮的 hover / 按压两态用户审阅后要求撤回，测试把输入的步进按钮按键盘控件处理；`.feedback` / `.textAction` 等既有规则未改。焦点态仍 `transition: none`，键盘即时不变。
