...
- 金额为 USD 整数美分，图片为本地 `/images/` 素材路径与正整数尺寸；alt 可为空以表达纯装饰。P5 已换为本地源稿导出与真实尺寸，不能把 Figma 临时 URL 写进契约实例。
...
- BMI 契约保留标题、字段 / 单位 / 结果标签与范围文字，新增 `sourcePreview` 显式保留源稿零输入、Female / imperial 选中与演示分数 56；这些是稿件字符串，不是服务端计算结果。C-05 区间标签已改为 18.5–<25 / 25–<30；C-04 已由 lib/bmi.ts 与 BmiCalculator 实施，sourcePreview 不参与结果计算。
...
- 2026-09-17 整体 review 完成：C-02/03/05/07/08/11 的共享数据修正落地；C-06 标题与 FAQ 首答按用户要求保留，原始重复答案留在源快照和 deviations。BMI sourcePreview 与两板正文差异仍保留。D-02 由未决目标改为已决 demo 契约；P5 页面已从唯一入口完成组装。
...
- `app/page.tsx` 通过唯一入口取数并传给 14 根区块；图片对象来自本地源稿资源 manifest，显式宽高匹配解码尺寸。
