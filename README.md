...
**Status:** Implementation complete: fourteen homepage sections, eleven reusable primitives, real source assets, a working BMI calculator and accessible menu/carousel/FAQ/marquees. Storybook contains 113 stories. Every push runs the quality gates, the browser acceptance suite and the eleven-width scan on Linux through [GitHub Actions](#continuous-integration); [P5 evidence](docs/p5-review.md) records the design review.
...
| §4D interaction states    | Hover, focus-visible and pressed forced on every visible control, with a transition on pointer feedback | `tests/acceptance/states.spec.ts`            |
...
| NumberField      | native number input; Focus / Invalid / ExternalError / Disabled / hint; stepper Hover / Pressed; sm / md  | 9       |
